...
- **Day 195** ✅ — Recs QA + relevance tuning: 7 Vitest tests exercise the *real* shared scorer (`recs-core.js`) via an in-memory Firestore fake; tuned relevance = exclude coming-soon titles + a gentle recency tie-breaker. Function redeployed, demo recs refreshed. `docs/RECS_QA.md`.
...
- **Day 200** ✅ — Achievement engine: nightly `awardBadges` (europe-west4) evaluates 7 badges from cheap signals (completed-watch count, follows count, `stats.reviewsWritten`, `stats.premieresAttended`, `referralCount`) → `achievements/{uid}/badges/{badgeId}` + notifies once. Counter triggers `onReviewWritten`/`onPremiereAttended`; Recruiter also immediate on referral. Rules: function-write/signed-in-read. Earned badges shown on web account page + Android profile (earned vs locked). Deployed.
- **Day 201** ✅ — Stats: nightly `buildStats` (europe-west4) rolls up `stats/{uid}` — totalWatchMinutes (counts partial views: duration × progress), moviesCompleted, topGenres — merged so the trigger-kept reviewsWritten/premieresAttended survive. Stat tiles (not charts: single headline numbers) + top-genre chips on the web account page **and public profile**, alongside badges; Android profile gains the same stats row and its hardcoded "watched 0" is now real. Deployed.
- **Day 202** ✅ — Light gamification: `buildStats` now also derives a day **streak** from `lastActiveAt` (counted once per calendar day via `streakLastDay`, so a re-run cannot inflate it) and **weekly goal** progress (movies finished in the last 7 days vs `weeklyGoal`, default 3). Streak tile + capped goal meter on web account page and Android profile. **Shareable stats card**: `/api/stats-card/{uid}` renders a 1200×630 PNG via next/og, with Share (Web Share, attaching the file where supported) + Download.
...
- **Day 208** ✅ — Hardening → `docs/V13_HARDENING.md`. **Closed the top gap: referral reward chain verified end to end against prod** (`seed-referrals-demo.mjs` → deployed `onReferralCreated`: referralCount 0→3, earlyAccess, Recruiter badge, 3 notifs; fraud guard flags the 2 shared-IP referrals). Functions robustness + rules audit (all unforgeable data is function-write-only) + client crash-surface review. Fix: `/api/stats-card` now cache-controlled (public, CPU-bound image). Web tsc+lint+22 tests; Android assembleDebug.
- **Day 209** ✅ — Beta/fix pass → `docs/V13_RELEASE_READINESS.md`. **Production build verified** (`next build`, exit 0 — all v1.3 routes compile; catches server-component errors tsc/lint miss), 22 Vitest green, Android assembleDebug, all functions+rules deployed, reward chains verified live. Fix carried: stats-card caching. Go-live steps + smoke test + monitoring documented; the one open decision (merge to main vs local-source deploy) left to the user.
- **Day 210** ✅ — **🚀 v1.3 RELEASED.** Merged feat→main (fast-forward, 57 commits), deployed the web app to App Hosting (`firebase deploy --only apphosting`, filmatubelive) — rollout complete at https://filmatube--filmatubelive.europe-west4.hosted.app. **Smoke test green**: landing 200, /home 307→login, /invite/{uid} 200, /api/stats-card PNG 200 with Cache-Control. Functions+rules already live. Monitoring surfaces per `docs/V13_RELEASE_READINESS.md` (function summary logs, Crashlytics, App Hosting metrics). main pushed to origin.
