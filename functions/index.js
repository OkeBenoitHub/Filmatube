...
/**
 * Nightly content-overlap recommender — no ML.
 *
 * Reads the published catalogue once, then for each active user accumulates a taste profile
 * from the signals they already leave and scores every unseen movie by genre/person overlap.
 * Writes recs/{uid}. Deliberately a scheduled batch rather than a per-write trigger: recs
 * change slowly, and recomputing on every like would be far more expensive than one nightly
...
exports.buildRecommendations = onSchedule(
  ...
  async () => {
    ...
    for (const user of users.docs) {
      try {
        if (await buildRecsForUser(user.id, movies)) built += 1;
      } catch (e) {
        console.error(`recs: failed for ${user.id}`, e);
      }
    }
    console.log(`recs: built ${built}/${users.size} from a catalogue of ${movies.length}`);
  },
);
 ...
 */
async function buildRecsForUser(uid, movies) {
