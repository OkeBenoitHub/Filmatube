...
// ───────── referral rewards + achievements (v1.3, Days 199–202) ─────────
//
// Badge thresholds, award logic and the stats roll-up live in ./gamification-core.js so these
// handlers and scripts/run-gamification.mjs run the identical evaluation.
const { awardBadge, awardAllBadges, buildAllStats } = require("./gamification-core");
...
exports.onReferralCreated = onDocumentWritten(
  ...
  async (event) => {
    ...
    await awardBadge(db, FieldValue, referrerId, RECRUITER_BADGE);
...
//
// Thresholds + award logic live in ./gamification-core.js so these scheduled jobs and
// scripts/run-gamification.mjs run the identical evaluation.
...
exports.awardBadges = onSchedule(
  ...
  async () => {
    const { awarded, total } = await awardAllBadges(db, FieldValue, notifyUser);
    console.log(`badges: awarded ${awarded} across ${total} users`);
  },
);
...
/**
 * Nightly stats roll-up → `stats/{uid}` (Days 201–202). Logic in ./gamification-core.js.
 */
exports.buildStats = onSchedule(
  { schedule: "every 24 hours", region: "europe-west4" },
  async () => {
    const { built, total, catalogue } = await buildAllStats(db, FieldValue);
    console.log(`stats: built ${built}/${total} from a catalogue of ${catalogue}`);
  },
);
